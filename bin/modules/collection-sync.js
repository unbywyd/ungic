...
class Collection {
    constructor(config={}) {
        ...
        this._change = (event, data, silent) => {
            if(silent) {
                event = event + '_silent';
            }
    ...
    _insert(model, options) {
        ...
        if(!options.silent) {
            ...
            this._change('added', model, true);
        }
        return model;
    }
    _update(id, attributes, options) {
        ...
        if(!options.silent && _.keys(model.changed).length) {
            ...
            this._change('updated', model, true);
        }
        return model;
    }
        ...
        for(let model of models) {
            if(this._isModel(model)) {
                if(!this.has(model.id) && options.add) {
                    ...
                    let updated = this._update(model.id, model.get(), options);
                ...
                if('function' == typeof this._change.attrPrehandler) {
                    let attrs = this._change.attrPrehandler(model);
                    if(attrs) {
                        model = attrs;
                    }
                }
                if(!this.has(model.id) && options.add) {
                    model = this._create(model, options);
                    if(model) {
                        let inserted = this._insert(model, options);
                        changed.push({
                            event: "added",
                    ...
                    let updated = this._update(model.id, _.omit(model, 'id'), options);
                    if(updated && _.keys(updated.changed).length) {
                        changed.push({
                            event: "updated",
                            model: updated
    ...
    reset(models, options={}) {
        ...
        let changed;
        if(models) {
            changed = this._set(models, {silent: true, add: true, remove: true, merge: true});
        }
        if(!options.silent) {
    ...
    create(attrs, options={}) {
        ...
        let model = this._create(attrs, options);
        if(model) {
            if(!options.silent) {
